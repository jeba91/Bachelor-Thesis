...
//  The Astar constructor setting creating a new object.
    public void Astar(){
        ...
        while(!done){
        	...
        	for(int i = 0; i< adjacentCells.size(); i++){
        		...
        		if(!openList.contains(currentAdj)){
        			currentAdj.Astar.setPrevious(current);
        			currentAdj.Astar.setFcost(current, currentAdj, eind);
        			openList.add(currentAdj);    			
        		}else{
        			if(currentAdj.Astar.getGcost() > currentAdj.Astar.calculateGcost(current, currentAdj)){
        				currentAdj.Astar.setPrevious(current);
        				currentAdj.Astar.setFcost(current, currentAdj, eind);
    				}
        		}    		
...
//  Function to find the lowest value in the list **TODO** Create a comparator to make this function useless
    private Cell lowestFInOpen() {
        ...
        for (int i = 0; i < openList.size(); i++) {
            if (openList.get(i).Astar.getFcost() < cheapest.Astar.getFcost()) {
...
//  Function that returns the path after reaching the goal state
    private List<Cell> calcPath(Cell start, Cell goal) {
	    ...
	    while(!done){
	    	...
	    	current = current.Astar.getPrevious();
